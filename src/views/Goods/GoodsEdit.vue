...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      let params = {
        ...
        goodsname,
        goodscategory,
        goodsfeature: JSON.stringify(goodsfeature), // 特色需要处理
        imgUrl,
        isPromotion,
        // 规格参数需要处理
        standard:
  ...
  created() {
    ...
    // 使用本地数据 回填表单
    // this.goodsAddForm.goodsname = row.goodsname; // 回填名称
    // this.goodsAddForm.goodscategory = row.goodscategory; // 回填分类
    // this.goodsAddForm.imgUrl = row.imgUrl; // 回填图片
    // this.goodsAddForm.isPromotion = row.isPromotion; // 回填促销
    // this.goodsAddForm.goodsdesc = row.goodsdesc; // 回填描述
    // // 多回填一个id （多给goodsAddFrom一个id， 修改的时候需要使用）
    // this.goodsAddForm.id = row.id;

    // 优化代码
    for (let key in row) {
      if (key !== "ctime" && key !== "goodsfeature" && key !== "standard") {
        this.goodsAddForm[key] = row[key];
      }
    }

    this.goodsAddForm.goodsfeature = JSON.parse(row.goodsfeature); // 回填特色
    let standard = JSON.parse(row.standard); // 规格

    // 判断是多规格还是单规格
    if (standard.length > 1) {
      this.goodsAddForm.goodsstandard = "多规格";
      this.goodsAddForm.tableData = standard;
      console.log(standard);
    } else {
      this.goodsAddForm.goodsstandard = "单规格";
      this.goodsAddForm.packingexpense = standard[0].packingexpense;
      this.goodsAddForm.goodsprice = standard[0].goodsprice;
    }
  }
};
</script>
